...
class LaravelCacheSpec extends ObjectBehavior
{
    ...
    public function it_is_initializable(Config $config)
    {
        ...
        $this->shouldHaveType('Leifel\ForrestAdmin\Interfaces\StorageInterface');
    }
    ...
    public function it_should_allow_storing_cache_forever(Cache $cache, Config $config)
    {
        ...
        $config->get('forrestadmin.storage.store_forever')->shouldBeCalled()->willReturn(true);
    ...
    {
        $config->get(Argument::any())->shouldBeCalled()->willReturn(10);
        $config->get('forrestadmin.storage.store_forever')->shouldBeCalled()->willReturn(false);
    ...
    {
        $config->get('forrestadmin.storage.path')->shouldBeCalled()->willReturn('path');
        $config->get('forrestadmin.storage.expire_in')->shouldBeCalled()->willReturn('asdfa');
        $config->get('forrestadmin.storage.store_forever')->shouldBeCalled()->willReturn(false);
    ...
    {
        $config->get('forrestadmin.storage.path')->shouldBeCalled()->willReturn('path');
        $config->get('forrestadmin.storage.expire_in')->shouldBeCalled()->willReturn(-15);
        $config->get('forrestadmin.storage.store_forever')->shouldBeCalled()->willReturn(false);
        $cache->put(Argument::any(), Argument::any(), 20)->shouldBeCalled();
    ...
    {
        $config->get('forrestadmin.storage.path')->shouldBeCalled()->willReturn('path');
        $config->get('forrestadmin.storage.expire_in')->shouldBeCalled()->willReturn('45');
        $config->get('forrestadmin.storage.store_forever')->shouldBeCalled()->willReturn(false);
