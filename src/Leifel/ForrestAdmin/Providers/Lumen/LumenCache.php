...
use Leifel\ForrestAdmin\Exceptions\MissingKeyException;
use Leifel\ForrestAdmin\Providers\Laravel\LaravelStorageProvider as StorageProvider;
...
class LumenCache extends StorageProvider
{
    ...
    {
        $this->cache            = $cache;
        $this->path             = $config->get('forrestadmin.storage.path');
        $this->storeForever     = $config->get('forrestadmin.storage.store_forever');
        $this->expirationConfig = $config->get('forrestadmin.storage.expire_in');
        $this->setMinutes();
    }
